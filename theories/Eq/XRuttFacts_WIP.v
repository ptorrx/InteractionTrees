...
Variant rrcompose {R1 R2 R3 : Type}
  (RR12  : R1 -> R2 -> Prop)
  (RR23  : R2 -> R3 -> Prop)
  (r1 : R1) (r3 : R3) : Prop :=
...
Definition NoCut {E: Type -> Type} {T: Type} (e : E T) := true.
...
Lemma rutt_trans {E1 E2 E3: Type -> Type} {R1 R2 R3 : Type}
  ...
  (RR12 : R1 -> R2 -> Prop)
  (RR23 : R2 -> R3 -> Prop)
  (F: itree E1 R1 -> itree E2 R2)
  (CND: forall t1, rutt (@EE1) (@EE2) REv12 RAns12 RR12 t1 (F t1) <->
              rutt (@EE1) (@NoCut E2) REv12 RAns12 RR12 t1 (F t1))
  t1 t2 t3 : 
  (*  (forall A (e2: E2 A), (EE2' _ e2 = false) -> (EE2 _ e2 = false)) -> *)
  t2 = (F t1) ->
...
Proof.
  ...
  dependent induction INL.

  { rewrite <- x0.
    rewrite <- x in INR.

    dependent induction INR; intros; try discriminate.
    ...
    { destruct DEC as [EQ | EQ]. 
      { destruct EQ as [m3 ?]; subst.
        { rewrite <- x0.
          ...
          assert (paco2 (rutt_ EE2 EE3 REv23 RAns23 RR23) bot2 m2 m3) as H1.
          { eapply rutt_inv_Tau. pstep. red. auto. }

          eapply (CIH m1 m2 m3); eauto with paco.
      ...
      { rewrite <- x0.
        ...
        - econstructor; eauto. 
          ...
          hinduction H1 before CIH; intros;
            try (exfalso; eapply EQ; eauto; fail).
          
          { remember (RetF r1) as ot.
            hinduction H before CIH; intros; inv Heqot; eauto with paco itree.
            * by constructor; econstructor; eauto.
            * eapply EqCutL; eauto.
              by constructor; eapply IHruttF; eauto. }
              
          { remember (VisF e1 k1) as ot.
            hinduction H before CIH; intros; try discriminate.            

            { dependent destruction Heqot.
              constructor; eauto. 
              + by econstructor; eauto.
              + move=> a b /(_ _ _ H1 H5) [t2 HA12 HA23].

                specialize (H2 a t2 HA12).
                specialize (H6 t2 b HA23).
                pclearbot. right.
                eapply (CIH (k0 a) (k1 t2) (k3 b)); eauto.
                admit.
             }   
  ...
  { rewrite <- x0.
    ...
    { dependent destruction Heqot.
      ...
      + move=> a b /(_ _ _ H1 H5) [t2 HA12 HA23].
        ...
        specialize (H6 t2 b HA23).
        pclearbot. right.
        eapply (CIH (k1 a) (k2 t2) (k3 b)); eauto.
        admit.  
    }
  ...
  { rewrite <- x. eapply EqCutL; eauto. }

  { rewrite <- x. constructor. eapply IHINL; eauto.
  ...
  { rewrite <- x in INR.
    ...
    + econstructor; eapply IHINL; pclearbot; punfold H.
      (* PROBLEM *)
      admit.
...
Lemma rutt_trans1 {E1 E2 E3: Type -> Type} {R1 R2 R3 : Type}
  (EE1: forall X, E1 X -> bool)
  (EE2: forall X, E2 X -> bool)
  (EE3: forall X, E3 X -> bool)
  (REv12 : prerel E1 E2)
          ...
          { remember (VisF e1 k1) as ot.
            ...
            { dependent destruction Heqot.
              ...
              + move=> a b /(_ _ _ H1 H5) [t4 HA12 HA23].
                ...
                specialize (H2 a t4 HA12).
                specialize (H6 t4 b HA23).
                pclearbot. right.
                eapply (CIH (k0 a) (k1 t4) (k3 b)); eauto.
                admit.
             }   
  ...
  { rewrite <- x0.
    ...
    { dependent destruction Heqot.
      ...
      + move=> a b /(_ _ _ H1 H5) [t4 HA12 HA23].
        ...
        specialize (H6 t4 b HA23).
        pclearbot. right.
        eapply (CIH (k1 a) (k2 t4) (k3 b)); eauto.
        admit.  
    }
...
(*
  { assert (DEC: (exists m3, ot3 = TauF m3) \/ (forall m3, ot3 <> TauF m3)).
    ...
    { destruct DEC as [EQ | EQ]. 
      { destruct EQ as [m3 ?]; subst.
        { rewrite <- x in INR.
          eapply EqTauR. eapply IHINL; eauto.
          (* PROBLEM *)

Abort.          
 *)

Lemma rutt_trans2 {E1 E2 E3: Type -> Type} {R1 R2 R3 : Type}
  (EE1: forall X, E1 X -> bool)
  (EE2: forall X, E2 X -> bool)
  (EE3: forall X, E3 X -> bool)
  (REv12 : prerel E1 E2)
