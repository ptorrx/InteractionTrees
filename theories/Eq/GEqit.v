...
Section eqit.
  ...
  | EqErrL: forall A (e1 : E1 A)
                   ...
                   (hh : In E1 ErrorEvs)
                   (CHECK: b3),  
      eqitF b1 b2 b3 b4 vclo sim (VisF e1 k1) ot2
  | EqErrR: forall A (e2 : E2 A)
                   (k2: A -> itree E2 R2)
                   (ot1: itree' E1 R1)
                   (hh: In E2 ErrorEvs)
                   (CHECK: b4),  
      eqitF b1 b2 b3 b4 vclo sim ot1 (VisF e2 k2) 
  | EqTauL: forall (t1: itree E1 R1) (ot2: itree' E2 R2)
        (CHECK: b1)
        (REL: eqitF b1 b2 b3 b4 vclo sim (observe t1) ot2),  
      eqitF b1 b2 b3 b4 vclo sim (TauF t1) ot2 
  | EqTauR: forall ot1 t2
        (CHECK: b2)
        (REL: eqitF b1 b2 b3 b4 vclo sim ot1 (observe t2)), 
      eqitF b1 b2 b3 b4 vclo sim ot1 (TauF t2).
...
*)
  ...
  Lemma eqit_idclo_mono: monotone2 (@id (itree E1 R1 -> itree E2 R2 -> Prop)).
  Proof. unfold id. eauto. Qed.

  Hint Resolve eqit_idclo_mono : paco.

  Definition eqit b1 b2 b3 b4 : itree E1 R1 -> itree E2 R2 -> Prop :=
...
Lemma eqitF_inv_VisF_r {E1 E2 R1 R2}
  (RR : R1 -> R2 -> Prop)                      
  (PREf : forall A1 A2, (A1 -> A2 -> Prop) -> E1 A1 -> E2 A2 -> Prop)
  (ErrorEvs: list (Type -> Type)) 
  {b1 b2 b3 b4 vclo sim}
  (t1: itree' E1 R1) X2
  (e2 : E2 X2) (k2 : X2 -> itree E2 R2) :
  eqitF RR PREf ErrorEvs b1 b2 b3 b4 vclo sim t1 (VisF e2 k2) ->
    (exists X1 (LR: X1 -> X2 -> Prop)
            (e1: E1 X1) k1,
        t1 = VisF e1 k1 /\ PREf X1 X2 LR e1 e2 /\
          forall v1 v2, LR v1 v2 -> vclo sim (k1 v1) (k2 v2)) \/
    (b1 /\ exists t1', t1 = TauF t1' /\
                           eqitF RR PREf ErrorEvs b1 b2 b3 b4 vclo sim
                             (observe t1') (VisF e2 k2)) \/
    (b3 /\ In E1 ErrorEvs /\ exists X1 (e1: E1 X1) k1,
          t1 = VisF e1 k1) \/
    (b4 /\ In E2 ErrorEvs).
Proof.
  intros.
