...
(* strict version: relations only on effects *)  
  ...
  | EqVis : forall (A B : Type) (e1 : Ef1 A) (e2 : Ef2 B)
                   (k1 : A -> itree E1 R1)
                   (k2 : B -> itree E2 R2),
      REv e1 e2 ->
  ...
  Proof.
    ...
    - dependent destruction H.
      + eapply IsoInjection1 in x; eauto.
        inv x.
        left; eauto.
      + right; left; eauto.
      + eapply IsoCongruence21 in x; eauto; intuition.   
  ...
  Proof.
    ...
    - eapply IsoInjection1 in x0.
      eapply IsoInjection1 in x.
      inv x; assumption.
    - eapply IsoCongruence21 in x; eauto; intuition.
    - eapply IsoCongruence21 in x; eauto; intuition.  
  Qed.
...
Proof.
  ...
  - remember (VisF (effect EE1 e1) k1) as x. red.
    hinduction EQVl before r; intros; subst; try discriminate;
      try (constructor; eauto; fail).
    remember (VisF (effect EE2 e2) k3) as y.
